...
- **Report schema versioning**: All report/export endpoints include a `report_version` field in the payload (e.g., `"report_version": "1.0"`).
  - **Minor version** (1.0 → 1.1): Add new fields only (backward compatible)
  - **Major version** (1.0 → 2.0): Breaking changes (remove/rename fields)
  - Clients should check `report_version` and handle different formats gracefully.
  - See implementation: `REPORT_VERSION` constant in `services/api/app.py`
...
- Report endpoints:
  - `GET /daily/summary?date=YYYY-MM-DD` - Aggregated daily health summary
  - `GET /export/profile` - Profile export report
  - Both include `report_version` field for schema evolution

**Intensive testing gate**
- [ ] Time-series inserts scale (load test 10k inserts)
- [ ] Aggregation queries return correct daily summaries
- [ ] Report responses include `report_version` field
