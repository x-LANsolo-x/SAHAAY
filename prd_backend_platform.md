...
- `GET /export/profile` - Profile export report (includes `report_version`)
- `GET /daily/summary?date=YYYY-MM-DD` - Daily health summary (includes `report_version`)

**Note on API versioning:** Report/export endpoints include a `report_version` field in the payload to enable schema evolution and backward compatibility. Clients should check this version before parsing the response.
